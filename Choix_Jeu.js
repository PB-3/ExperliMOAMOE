...
function choix_scenario(event) {
  ...
  if (selectedChoice === 'MOA') {
    ...
    window.location.href=grandParentUrl+'/MOE-code/Jeu_MOE.php';
  ...
  // Afficher le chemin d'accès au dossier parent (last)
    console.log(grandParentUrl);  
    window.location.href=grandParentUrl+'/prince2_pmbok/Jeu_PRINCE2_PMBOK.php';
  }
    ...
    // Afficher le chemin d'accès au dossier parent (last)
    console.log(grandParentUrl);
    var progress = document.querySelector('.progress').style.display = 'hidden';
    var bar = document.querySelector('.progress-bar').style.display='hidden';
    window.location.href=grandParentUrl+'/norme_qualite/norme_qualite.php';
